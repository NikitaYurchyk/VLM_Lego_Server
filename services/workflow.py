...
async def call_llm_with_timeout(llm, messages, timeout_seconds=60):
    ...
    try:
        ...
        raise TimeoutError(f"The AI provider took too much time to respond (>{timeout_seconds} seconds). Please try again or check your connection.")
...
class WorkflowManager:
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        workflow.add_node("classify_regular_instruction", self.classify_regular_instruction)
    ...
    async def classify_message(self, state: AssemblyState):
        ...
        prompt = f"""Classify the message. If the message contains next, next step, previous, prev, reset, zoom in, zoom out or show image, they should be classified it as 'regular'. If the message contains word check, it should be classified as 'check'.
...
Respond with ONLY one word: either "check" or "regular"""
        ...
        try:
            result = await call_llm_with_timeout(
                self.agent.classify_msg_llm,
                [HumanMessage(content=prompt)]
            )
            
            # Parse simple text response
            response_text = result.content.strip().lower()
            message_type = "check" if "check" in response_text else "regular"
            ...
            state.message_type = "check" if "check" in user_message.lower() else "regular"
...
Respond with ONLY the classification category."""
        ...
        try:
            result = await call_llm_with_timeout(
                self.agent.classify_regular_msg_llm,
                [HumanMessage(content=prompt)]
            )
    ...
    async def zoom_in(self, state: AssemblyState) -> AssemblyState:
        ...
        has_multiple_refs = hasattr(state, 'has_multiple_images') and state.has_multiple_images
    ...
    async def show_image(self, state: AssemblyState) -> AssemblyState:
        ...
        if num_images > 0:
            if num_images == 1:
                state.feedback = f"Here's the reference image for step {state.current_step}"
            else:
            ...
            state.feedback = f"No reference images available for step {state.current_step}"
        return state
    ...
    async def get_current_step(self, state: AssemblyState) -> AssemblyState:
        ...
        try:
            # Get instructions for current step from database
    ...
    async def process_image(self, state: AssemblyState) -> AssemblyState:
        ...
        try:
            ...
            if sequential_groups and len(sequential_groups) > 0:
                ...
                images_base64 = [img for img in await asyncio.gather(*image_tasks) if img is not None]
        ...
        try:
            ...
            if results["documents"] and results["documents"][0]:
                ...
                if results["metadatas"] and results["metadatas"][0]:
                    ...
                    print(f"Retrieved metadata for step: {metadata.get('step', 'unknown')}")
                    
                    # Get primary reference image
                    if "reference_image" in metadata:
                        state.reference_image = metadata["reference_image"]
                        print(f"Primary reference image found for step {metadata.get('step', 'unknown')}")
                    else:
                    ...
                    if "reference_images" in metadata and metadata["reference_images"]:
                        ...
                        print(f"Found {len(state.reference_images)} additional reference images for step {metadata.get('step', 'unknown')}")
                    else:
                        state.reference_images = []
                        print("No additional reference images found")
                    
                    # Check if this step has multiple reference images
                    state.has_multiple_images = metadata.get("has_multiple_images", False)
                    print(f"Has multiple images: {state.has_multiple_images}")
                    
            else:
                state.instruction_text = f"Complete step {state.current_step} of the LEGO assembly."
                state.reference_images = []
    ...
    async def compare_assembly(self, state: AssemblyState) -> AssemblyState:
        ...
        if not state.current_images or len(state.current_images) == 0 or not state.instruction_text:
            return state
        ...
        **Action Items:** Provide concrete next steps when assembly needs correction
        ...
        try:
            ...
            try:
                ...
                state.is_step_complete = comparison_result.get("step_complete", False)
    ...
    async def generate_feedback(self, state: AssemblyState) -> AssemblyState:
        ...
        if not state.comparison_result:
            state.feedback = "Please ensure you're following the instructions carefully."
            return state
